...
// len for total resultant prefix len
pub fn gen_ip(
    prefix: IpNetwork,
    namespace: String,
    name: Option<String>,
    len: u8,
) -> (IpNetwork, Option<IpAddr>) {
    ...
    match prefix.network() {
        ...
        IpAddr::V4(pre_net_ip) => {
            ...
            let filtered = bitwise_op(&maskv4.octets(), ns_hash, |a, b| !a & b);
            let combined = bitwise_op(&pre_net, &filtered, |a, b| a | b);
            let ns_ip = Ipv4Addr::from(combined).into(); // | prefix | hash |
            let ns_net: Ipv4Network = Ipv4Network::new(ns_ip, len).unwrap();

            if name.is_none() {
                (ns_net.into(), None)
            } else {
                let name_hash = gen_bytes(name.unwrap());
                let filtered = bitwise_op(&ns_net.mask().octets(), &name_hash.as_bytes()[..=3].try_into().unwrap(), |a, b| !a & b);
                let ip_final = bitwise_op(&ns_net.network().octets(), &filtered, |a, b| a | b);

                (ns_net.into(), Some(Ipv4Addr::from(ip_final).into()))
            }
        }
        IpAddr::V6(pre_net_ip) => {
            ...
            if name.is_none() {
                ...
                (ns_net.into(), Some(Ipv6Addr::from(ip_final).into()))
            }
        }
    }
}
...
fn ips() {
    ...
    let net = dbg!(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), None, 15).0.network());
    let ip1 = dbg!(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), Some("app1".to_string()), 15)).1.unwrap();

    print_ip(net);
    print_ip(ip1);
    print_ip(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), Some("app2".to_string()), 15).1.unwrap());
    print_ip(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), Some("app4".to_string()), 15).1.unwrap());
    print_ip(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), Some("app5".to_string()), 15).1.unwrap());
    print_ip(gen_ip(IpNetwork::new([12,255,0,0].into(), 12).unwrap(), "test".to_string(), Some("app7".to_string()), 15).1.unwrap());
}
...
fn test_ipgen() -> Result<()> {
    let c1 = gen_ip("10.254.0.0/8".parse()?, "cafe".to_string(), None, 16).0;
    let c2 =  gen_ip("10.254.0.0/8".parse()?,"cafe".to_string(),None, 16).0;
    let c1n = c1.network();
    let c2n = c2.network();
    assert_eq!(c1n, c2n);
    dbg!(c1n);
    ...
    let c1 = gen_ip("10.254.0.0/8".parse()?, "cafe".to_string(), None, 16).0;
    let c2 =gen_ip("10.254.0.0/9".parse()?, "cafe".to_string(), None, 16).0;
    dbg!(c1);
    dbg!(c2);
    let c1n = c1.network();
    let c2n = c2.network();
